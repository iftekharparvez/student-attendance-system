...
class Student{
	...
	public function insertStudent($name,$roll){
		...
		if(empty($name) || empty($roll)){
			...
			$std_insert = $this->db->insert($stu_query);

			$att_query = "INSERT INTO tbl_attendance(roll) VALUES('$roll')";
			$std_insert = $this->db->insert($att_query);
	...
	public function insertAttendance($cur_date, $attend = array()){
		...
		if($data_insert){
			$msg = "<div class='alert alert-success'><strong>Success !</strong> Attendance successfully</div>";
			return $msg;
		}
		else{
	...
	public function updateAttendance($dt, $attend){
		...
		if($data_update){
			$msg = "<div class='alert alert-success'>Attendance Updated successfully</div>";
			return $msg;
		}
		else{
